...
contract BlockchainForestDefi is Ownable {
   ...
   //--> USDT vers DAI
    ...
    function addliquidityToDaiUsdt(uint _amountDai) public returns (bool) {
        ...
        UniswapRouter.addLiquidity(usdtTokenAddress, daiTokenAddress, _amountDai, _amountDai, 1000, 1000, owner, now + 120);
        return true;
    ...
    //fonction remove
    function removeLiquidityfromDaiUsdt(uint _amountDai) public returns (bool) {
        IUniswapV2Router02 UniswapRouter = IUniswapV2Router02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
        IERC20 daiInstance = IERC20(daiTokenAddress);
        IERC20 usdtInstance = IERC20(usdtTokenAddress);
        daiInstance.approve(address(UniswapRouter), _amountDai);
        usdtInstance.approve(address(UniswapRouter), _amountDai);
        uniswapRouter.removeLiquidity(daiTokenAddress, usdtTokenAddress, _amountDai, 1000, 1000, owner, now + 120);
        return true;
